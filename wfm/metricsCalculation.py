...
class MetricsCalculation:
    ...
    def serialize_data(self, df_dhm_on_date):
        output_objects = []
    ...
    def calculate_output_data(self):
        ...
        df_dhm_on_hour['covering_sum'] = df_dhm_on_hour.groupby(['date', 'duty'])['covering_value'].transform("sum").astype('float')
        df_dhm_on_hour['breaks_sum'] = df_dhm_on_hour.groupby(['date', 'duty'])['breaks_value'].transform("sum").astype('float')
        df_dhm_on_hour['overcovering_value'] = df_dhm_on_hour.groupby(['date', 'duty'])['overcovering'].transform("sum").astype('float')
        ...
        metrics_serializer = self.serialize_data(df_dhm_on_date)
